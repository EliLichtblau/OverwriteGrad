...
from OverwriteGrad.squaregradFunctions import add, mul
...
class Dropout(torch.nn.Dropout):
    def __init__(self, p: float = 0.5) -> None:
        super().__init__(p, False) # inplace not allowed
...
class Normalization(torch.nn.Module):
    def __init__(self, base_module: torch.nn.Module, normalized_shape: List[int], affine: bool = True):
...
class MultiheadAttention(torch.nn.MultiheadAttention):
    def __init__(self, embed_dim, num_heads, dropout=0, bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, batch_first=False, device=None, dtype=None) -> None:
        ...
        # nothing else is a layer don't need to overwrite
    ...
    Outputs:
        ...
        assert not any_nested, ("MultiheadAttention does not support NestedTensor outside of its fast path. " +
                                f"The fast path was not hit because 'fast path removed'")
...
# Everything needs to be over written - much easier to just
# do this by hand
class TransformerDecoderLayer(torch.nn.Module):
    __constants__ = ['batch_first', 'norm_first']
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1, activation: Callable[[Tensor], Tensor] = F.relu, layer_norm_eps: float = 0.00001, batch_first: bool = False, norm_first: bool = False, device=None, dtype=None) -> None:
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None,
        ...
        x = tgt
        if self.norm_first:
            x = add(x, self._sa_block(self.norm1(x), tgt_mask, tgt_key_padding_mask))
            x = add(x, self._mha_block(self.norm2(x), memory, memory_mask, memory_key_padding_mask))
            x = add(x, self._ff_block(self.norm3(x)))
        else:
            x = self.norm1(add(x, self._sa_block(x, tgt_mask, tgt_key_padding_mask)))
            x = self.norm2(add(x, self._mha_block(x, memory, memory_mask, memory_key_padding_mask)))
            x = self.norm3(add(x, self._ff_block(x)))
